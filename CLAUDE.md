...
- `Web` - optional, read-only event browser. Framework-agnostic Rack app (`DcbEventStore::Web`, `require "dcb_event_store/web"` - NOT auto-loaded, adds no runtime dep) mountable in Rails/Sinatra/bare Rack. `ReadModel` (PG glue: count/head/newest-first paging/find, reusing `Store::SqlBuilder` + `RowMapper`), `Router` (route dispatch), `ListParams` (pure paging + Query building - the mutation-tested part), `View` (ERB rendering, `web/views/*.erb`). Connection via `Web.connection = <pg conn or callable>`, or auto-borrows ActiveRecord's. Routes: `GET /` (list + type/tag filter + paging), `GET /events/:position` (detail).
