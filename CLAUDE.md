...
- `test/concurrency/` - concurrency tests (PG): `test_concurrent_append.rb` races whole appends, `test_in_flight_append.rb` pins one open mid-transaction
- Snapshot tests: `test/support/snapshot_store_contract.rb` (the snapshot store interface, run by `test/unit/test_in_memory_snapshot_store.rb`, `test/sqlite/test_sqlite_snapshot_store.rb`, `test/integration/test_postgres_snapshot_store.rb`) and `test/support/snapshot_decision_model_contract.rb` (`DecisionModel.build(snapshots:)` on every backend, included by the three DecisionModelContract runners, which define `build_snapshot_store`); `test/unit/test_snapshot.rb`, `test_snapshot_instrumentation.rb`, `test_query_key_collisions.rb` (regression: `Query#to_s` is not a safe cache key)
- `test/support/` - shared test infra: `postgres_database.rb` (`PostgresDatabaseHelper`: connection, schema, `build_store`), `sqlite_database.rb` (`SqliteDatabaseHelper`: tempfile database, schema, `build_store`, extra connections) and the backend contracts `store_contract.rb` (append/read/read_from/pagination/instrumentation), `special_characters_contract.rb`, `client_contract.rb`, `decision_model_contract.rb`, `upcaster_contract.rb`, `import_export_contract.rb` (`Store#import` + `EventFile` round trips), plus `in_memory_equivalence_contract.rb` (same scripted operations on the backend and on InMemoryStore, results compared). Each contract is a module run against every backend: PG via `test/integration/`, SQLite via `test/sqlite/test_sqlite_store.rb`, InMemory via `test/unit/test_in_memory_store.rb`. Including classes set `@store` in setup and define `build_store(upcaster: nil)`
...
- `SqlStore` - abstract base for SQL backends: instrumentation, paginated read, `last_position`, append orchestration, subscribe loop; subclasses implement the hooks (`with_write_transaction`, `acquire_locks!`, `count_matching`, `insert_event`, `fetch_batch`, `max_position`, `notify_appended`, `listen`/`unlisten`/`wait_for_append`)
- `PostgresStore` - low-level PG operations (advisory locks, single-statement conditional append via CTE, LISTEN/NOTIFY). `LockKeys.for(events, condition)`: every append takes key 0 shared plus one exclusive crc32 key per tag its events carry or its condition names; a condition naming no tag (type-only item or `Query.all`) takes key 0 exclusive instead. Covers both directions of a writer/condition race on one tag (issue #42; `test/concurrency/test_in_flight_append.rb` holds an append open mid-transaction and checks the racing one blocks). Was named `Store`; `store.rb` keeps `Store`, `Schema` and `PgArrayCodec` as aliases marked with `deprecate_constant` (covered by `test/unit/test_deprecated_aliases.rb`)
...
- `Projection` / `DecisionModel` - higher-level abstractions. `DecisionModel.build(store, snapshots: snapshot_store, **projections)` takes `store.last_position` first and drops everything past it from the reads (so an event landing between two reads is neither folded nor guarded), reads one group per snapshot position (projections without a snapshot form the nil group), merges the reads by position, folds each projection from its snapshot state (`Projection#fold(events, from:)`), guards the condition at the highest position any group's read covered (its last event, or its snapshot position when it read nothing; never the head, which on PG can lie past an in-flight same-tag append) and writes each group's snapshots at its own coverage once that is `Snapshot#every` positions past them (`DecisionModel::Snapshotting`)
- `Snapshot` - per-projection snapshot config (`name`, `version` (required, no default: invalidation is explicit), `every`, `dump`/`load`); key = `"[epoch/]name/vN/#{query.fingerprint}"`, the fingerprint swapped for its SHA-256 hex when longer than 64 chars (`Query#fingerprint` is JSON of the items; `Query#to_s` is for logs and not collision-free), `DcbEventStore::Snapshots.epoch` a process-wide prefix that invalidates everything at once. `Snapshot.key_prefix(name, version)` / `.epoch_prefix` are what the stores' `purge(name:, keep_version:)` / `purge_other_epochs` match on (substr, not LIKE: SQLite's LIKE is case-insensitive). `Snapshots::InMemorySnapshotStore` / `PostgresSnapshotStore` / `SqliteSnapshotStore` (`lib/dcb_event_store/snapshots/`) share one interface (`fetch`, `fetch_many`, `store` with a forward-only upsert, `delete`, `clear`, `purge`, `purge_other_epochs`); the SQL ones use the `projection_snapshots` table that `<Backend>Store::Schema.create!` installs with the events schema (mutable, no append-only trigger). Contract: `test/support/snapshot_store_contract.rb`, `test/support/snapshot_decision_model_contract.rb`
- `SqliteStore::Dialect#tags_contain(params, tags, after:)` repeats the position bound inside the `event_tags` subquery, so a catch-up read after a snapshot scans only the tag's rows past it (`PostgresStore::Dialect` accepts `after:` and ignores it). `examples/snapshot_benchmark.rb` + `examples/SNAPSHOTS.md` measure snapshots against replay (an in-process materialized-stream cache was measured too and dropped)
- `Upcaster` - event schema migration on read
- `EventFile` - JSON Lines import/export for seeding/backup/backend copies (`export(store, io_or_path, query:, after:)`, `import(store, io_or_path, batch_size: 1000)` -> `ImportResult(read, imported, skipped)`, `encode`/`decode`/`each_event`). Only `type` required per line; unknown keys raise. Import goes through `store.import(sequenced_events)` (all three stores): keeps id/created_at/schema_version, fresh positions in order, skips stored ids, no condition; `SqlStore` hooks `lock_for_import!` (PG: key 0 exclusive) + `import_event`, dialects `import_sql`/`import_params`/`encode_timestamp`. `DcbEventStore::CLI` (`lib/dcb_event_store/cli.rb`, not loaded by the gem) backs `bin/dcb_events export|import`. Tests: `test/support/import_export_contract.rb` (all backends), `test/unit/test_event_file.rb`, `test/sqlite/test_cli.rb`
- `Notifications` / `StoreInstrumentation` / `LogSubscriber` / `RailsLogSubscriber` - observability: pub/sub instrumentation events (`append.dcb`, `read.dcb`, `subscribe.dcb`, `projection.dcb`, `decision_model.dcb`, `snapshot.dcb` from `DecisionModel::Snapshotting` around snapshot loads/writes; tested in `test/unit/test_snapshot_instrumentation.rb`) published through the global `DcbEventStore.instrumentation`; `subscribe.dcb` carries delivery lag, per-event or batched via `subscribe_instrumentation:` store option. `LogSubscriber` is a plain-logger adapter; `RailsLogSubscriber` renders events in ActiveRecord SQL-query style (no Rails dependency); `ActiveSupportInstrumentation` is a drop-in engine routing events through `ActiveSupport::Notifications` (shared engine contract: `test/support/instrumentation_contract.rb`); `AppsignalSubscriber` maps events to AppSignal custom metrics (lazy require, engine-agnostic). `Railtie` (loaded from `lib/dcb_event_store.rb` only when `Rails::Railtie` is defined, so the gem still carries no Rails dependency) does the Rails wiring at boot without an initializer in the app: swaps in `ActiveSupportInstrumentation` and attaches `RailsLogSubscriber` to `Rails.logger`, configurable through `config.dcb_event_store` (`instrumentation:`, `log:`, `logger:`, `pattern:`, `colorize:`; the attached adapter and its handle come back on the same options object). Tested in `test/unit/test_railtie.rb` against a config double plus subprocess checks that the file loads only inside Rails — which is why `test/unit/test_rails_log_subscriber.rb` saves and restores `::Rails` around its own stubs
