...
- `Projection` / `DecisionModel` - higher-level abstractions. `DecisionModel.build(store, snapshots: store, **projections)` takes `store.last_position` first, reads one group per snapshot position (projections without a snapshot form the nil group), merges the reads by position, folds each projection from its snapshot state (`Projection#fold(events, from:)`), guards the condition up to the head and writes snapshots there once the head is `Snapshot#every` positions past them (`DecisionModel::Snapshotting`)
