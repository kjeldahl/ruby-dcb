...
- `Snapshot` - per-projection snapshot config (`name`, `version`, `every`, `dump`/`load`); key = `"name/vN/#{query.fingerprint}"`, the fingerprint swapped for its SHA-256 hex when longer than 64 chars (`Query#fingerprint` is JSON of the items; `Query#to_s` is for logs and not collision-free). `Snapshots::InMemorySnapshotStore` / `PostgresSnapshotStore` / `SqliteSnapshotStore` (`lib/dcb_event_store/snapshots/`) share one interface (`fetch`, `fetch_many`, `store` with a forward-only upsert, `delete`, `clear`); the SQL ones use the `projection_snapshots` table that `<Backend>Store::Schema.create!` installs with the events schema (mutable, no append-only trigger). Contract: `test/support/snapshot_store_contract.rb`, `test/support/snapshot_decision_model_contract.rb`
