...
- Snapshot tests: `test/support/snapshot_store_contract.rb` (the snapshot store interface, run by `test/unit/test_in_memory_snapshot_store.rb`, `test/sqlite/test_sqlite_snapshot_store.rb`, `test/integration/test_postgres_snapshot_store.rb`) and `test/support/snapshot_decision_model_contract.rb` (`DecisionModel.build(snapshots:)` on every backend, included by the three DecisionModelContract runners, which define `build_snapshot_store`); `test/unit/test_snapshot.rb`, `test_materialized_streams.rb`, `test_snapshot_instrumentation.rb`, `test_query_key_collisions.rb` (regression: `Query#to_s` is not a safe cache key)
