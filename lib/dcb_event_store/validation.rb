module DcbEventStore
  ...
  module Validation
    ...
    TYPE_PATTERN = /\A[a-zA-Z][a-zA-Z0-9_:.\-]*\z/.freeze
    ...
    def self.validate_type(type)
      ...
      if type.empty?
        raise ValidationError.new(field: :type, message: "cannot be empty")
      end
      ...
      unless TYPE_PATTERN.match?(type)
        raise ValidationError.new(
          field: :type,
          message: "must start with a letter and contain only alphanumeric characters, underscores, colons, dots, or hyphens"
        )
      end
    ...
    def self.validate_tags(tags)
      ...
      tags.each_with_index do |tag, index|
        if tag.length > MAX_TAG_LENGTH
          raise ValidationError.new(field: :tags, message: "tag at index #{index} exceeds maximum length of #{MAX_TAG_LENGTH} characters")
        end
    ...
    def self.validate_data(data)
      ...
      unless data.is_a?(Hash)
        raise ValidationError.new(field: :data, message: "must be a Hash")
      end
      
      # Check size when serialized
      json_data = JSON.generate(data)
      if json_data.bytesize > MAX_DATA_SIZE
        raise ValidationError.new(field: :data, message: "exceeds maximum size of #{MAX_DATA_SIZE / 1024 / 1024} MB")
      end
